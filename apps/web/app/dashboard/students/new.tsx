      ...
      <div className="mx-auto max-w-3xl">
        ...
        <Card className="rounded-[28px] border-[#E5DED3] bg-white">
          <CardHeader className="p-6 sm:p-8">
            <CardDescription className="font-bold uppercase tracking-[0.18em] text-[#2563EB]">Students</CardDescription>
            <CardTitle className="text-4xl">Add Student</CardTitle>
            <CardDescription className="text-base text-[#6F6A62]">Dummy form state for adding a first student. Submit routes to the success screen.</CardDescription>
          </CardHeader>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {['Full name', 'Email address', 'Parent phone'].map((label) => (
              <label key={label} className="space-y-2 text-sm font-semibold">
                <span>{label}</span>
                <Input className="h-12 rounded-2xl border-[#E5DED3] bg-[#F7F4EE]" placeholder={label} />
              </label>
            ))}
            <label className="space-y-2 text-sm font-semibold">
              <span>Class</span>
              <Select defaultValue="grade-8a">
                <SelectTrigger className="h-12 rounded-2xl border-[#E5DED3] bg-[#F7F4EE]">
          ...
          <label className="mt-4 block space-y-2 text-sm font-semibold">
            <span>Notes</span>
            <Textarea className="min-h-28 rounded-2xl border-[#E5DED3] bg-[#F7F4EE]" placeholder="Learning needs, parent preference, or admin notes" />
          </label>
          <div className="mt-5 rounded-2xl border border-dashed border-[#E5DED3] bg-[#F7F4EE] p-5">
            <Upload className="mb-3 h-6 w-6 text-[#2563EB]" />
            <p className="font-semibold">Import CSV</p>
            <p className="text-sm text-[#6F6A62]">File upload placeholder for bulk student import.</p>
          </div>
